...
var formats = {
    standart_url: 'http://mtgtop8.com/format?f=ST&meta=52',
    modern_url: 'http://mtgtop8.com/format?f=MO&meta=52',
    legacy_url: 'http://mtgtop8.com/format?f=LE&meta=52',
    vintage_url: 'http://mtgtop8.com/format?f=VI&meta=52',
    commander_url: 'http://mtgtop8.com/format?f=EDH&meta=52'
};
...
var fetchStandardEvents = function (page, callback) {
    ...
    req.post(standart_url, {form: {cp: page}}, function (err, res) {
        if (err) return callback(err);
...
var fetchArchetypeEvents = function (format, page, callback) {
    var url = formats[format + '_url'];
    if (arguments.length === 2) return fetchArchetypeEvents(format, 1, page);

    req.post(url, {form: {cp: page}}, function (err, res) {
        if (err) return callback(err);
...
module.exports = {
    fetchArchetypeEvents: fetchArchetypeEvents,
